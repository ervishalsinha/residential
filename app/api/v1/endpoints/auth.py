...
def _normalize_holder_name(raw_value: str | None) -> str | None:
    if raw_value is None:
        return None
...
def _normalize_bank_name(raw_value: str | None) -> str | None:
    if raw_value is None:
        return None
    value = " ".join(raw_value.strip().split())
    return value or None
...
def update_owner_payment_settings(payload: OwnerPaymentSettingsUpdate, user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if payload.payment_upi_account_holder_name is not None:
        user.payment_upi_account_holder_name = _normalize_holder_name(payload.payment_upi_account_holder_name)
    if payload.payment_bank_account_holder_name is not None:
        user.payment_bank_account_holder_name = _normalize_holder_name(payload.payment_bank_account_holder_name)
    if payload.payment_bank_name is not None:
        user.payment_bank_name = _normalize_bank_name(payload.payment_bank_name)
    ...
    if user.active_payment_method == "upi" and (not user.payment_upi_id or not user.payment_upi_account_holder_name):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="UPI ID and account holder name are required when active_payment_method is 'upi'",
        )

    if user.active_payment_method == "bank" and (
        not user.payment_bank_account_holder_name
        or not user.payment_bank_name
        or not user.payment_bank_account_number
        or not user.payment_bank_ifsc
    ):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="Bank account holder name, bank name, account number and IFSC are required when active_payment_method is 'bank'",
        )
